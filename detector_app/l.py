...
from PyPDF2.generic import DictionaryObject, ArrayObject, TextStringObject, ByteStringObject, NameObject, FloatObject, NumberObject
import PIL.Image
import zlib
import base64
import hashlib
...
class AdvancedPDFSteganography:
    ...
    def _hide_data_with_technique(self, pdf_reader, data_fragments):
        ...
        try:
            ...
            # Copy all pages to writer and ensure they are properly cloned
            for page_num in range(len(pdf_reader.pages)):
                # Get the page and ensure it's properly cloned
    ...
    def _hide_in_metadata(self, pdf_writer, fragments):
        ...
        try:
            ...
            # Convert to DictionaryObject for PyPDF2 - FIXED: Use NameObject for keys
            metadata_dict = DictionaryObject()
            for key, value in metadata.items():
                # Convert string key to NameObject (proper PDF object format)
                name_key = NameObject('/' + key)
                metadata_dict[name_key] = TextStringObject(str(value))
    ...
    def _hide_in_comments(self, pdf_writer, fragments):
        ...
        try:
            # Add annotations to the first page's /Annots array
            if len(pdf_writer.pages) == 0:
                print("[-] No pages in PDF writer")
                return None
            ...
            for i, fragment in enumerate(fragments):
                ...
                annotation.update({
                    ...
                    NameObject("/Rect"): ArrayObject([FloatObject(0), FloatObject(0), FloatObject(0), FloatObject(0)]),  # Invisible rectangle
                    NameObject("/Contents"): TextStringObject(f"ProcessingData_{i:03d}_{encoded_data}"),
                    NameObject("/F"): NumberObject(2),  # Hidden flag
                    NameObject("/StegoPart"): TextStringObject(str(i))  # Custom identifier
                })
                annots.append(annotation)

            print(f"[+] Hidden {len(fragments)} fragments in comments (as annotations on first page)")
            return pdf_writer
        ...
        try:
            ...
            for i, fragment in enumerate(fragments):
                ...
                custom_obj.update({
                    ...
                    NameObject("/Length"): len(encoded_data),
                    NameObject("/Filter"): NameObject("/ASCIIHexDecode"),
                    NameObject("/ProcessingData"): TextStringObject(encoded_data)
                })
    ...
    def _finalize_and_save(self, pdf_writer, output_path):
        ...
        try:
            ...
            # Ensure the writer is valid
            if pdf_writer is None:
                print("[-] Error: Invalid PDF writer object")
                return False

            # Write to a temporary BytesIO first to validate
            temp_buffer = io.BytesIO()
            pdf_writer.write(temp_buffer)
            temp_buffer.seek(0)

            # If we got here, the PDF is valid, write to file
            with open(output_path, 'wb') as output_file:
                output_file.write(temp_buffer.getvalue())
    ...
    def _extract_from_comments(self, pdf_reader):
        """Extract data fragments from PDF comments (annotations on pages)."""
        fragments = []
        try:
            for page in pdf_reader.pages:
                ...
                for annot in annots:
                    try:
                        ...
                        continue
            print(f"[+] Extracted {len(fragments)} fragments from comments (annotations)")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # Example 1: Hide PNG in PDF with encryption
    print("\nExample 1: Hiding PNG in PDF with encryption")
    success, stego_pdf = hide_png_in_pdf(
        pdf_path=pdf_input,
        png_path=png_input,
        password="123",
        technique="multi"  # Using multiple techniques for better security
    )
    
    if success:
        # Example 2: Extract PNG from PDF with encryption
        print("\nExample 2: Extracting PNG from PDF with encryption")
        success, extracted_png = extract_png_from_pdf(
            pdf_path=stego_pdf,
            password="123",
            technique="multi"
        )
